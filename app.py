...
def get_data_from_sheets():
    try:
        s = st.secrets
        # 見出しがあってもなくても辞書として読み込む
        creds_dict = dict(s["gcp_service_account"] if "gcp_service_account" in s else s)
        
        # 鍵（private_key）の形式を徹底的に掃除する
        if "private_key" in creds_dict:
            # 改行コードのズレを修正し、前後の余計なスペースを消す
            cleaned_key = creds_dict["private_key"].replace("\\n", "\n").replace(" ", "").replace("\n", "---NEWLINE---")
            cleaned_key = cleaned_key.replace("---NEWLINE---", "\n").strip()
            # 念のため、BEGIN/END行が壊れていないか再構築
            if "-----BEGINPRIVATEKEY-----" in cleaned_key.replace("\n", ""):
...
if not df_master.empty:
    ...
    st.info("スプレッドシートのデータを読み込んでいます。反映されない場合はリロードしてください。")
