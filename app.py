            ...
            with tab:
                ...
                for j, cat in enumerate(cats):
                    with cols[j]:
                        ...
                        val = st.selectbox(cat, ["選択なし"] + opts, key=f"sel_{i}_{j}")
                        day_plan[cat] = val
                selected_plan[d_label] = day_plan

        st.divider()
        user_memo = st.text_area("📝 フリーメモ", placeholder="牛乳、パン、金曜外食など")
        ...
        if st.button("こんだけ作成", type="primary", use_container_width=True):
            ...
            if raw_ings:
                ...
                for n, c in counts.items():
                    st.checkbox(f"{n} × {c}" if c > 1 else n, key=f"chk_{n}")
    else:
        st.error("menu.csvが見つかりません。GitHubで作成してください。")
        ...
        if st.form_submit_button("保存する"):
            if n_dish and n_ing and current_sha:
                ...
                if update_github_csv(new_df, current_sha):
                    st.success("保存完了！反映まで1分ほどお待ちください。")
                    st.cache_data.clear()
                else: st.error("保存失敗。設定を確認してください。")
            else: st.warning("未入力項目があります。")
    
    st.divider()
    st.markdown('<div class="thin-title">登録済みメニュー</div>', unsafe_allow_html=True)
